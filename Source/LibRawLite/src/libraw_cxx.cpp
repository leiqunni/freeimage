...
// ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ thumbnail ・ｽ・ｽ ・ｽ・ｽ・ｽ・ｽ・ｽ, ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ thumb_format ・ｽ ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ ・ｽ ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ・ｽ
...
void LibRaw::x3f_dpq_interpolate_af(int xstep, int ystep, int scale)
{
  ...
  for (int y = 0; y < imgdata.rawdata.sizes.height + imgdata.rawdata.sizes.top_margin; y += ystep)
  {
    ...
    if (y > imgdata.rawdata.sizes.raw_height - scale)
      break;
    uint16_t *row0 = &image[imgdata.sizes.raw_width * 3 * y];                // ﾐ斷ｰﾑ威ｰ ﾑ・びﾐｾﾐｺﾐｰ
    uint16_t *row_minus = &image[imgdata.sizes.raw_width * 3 * (y - scale)]; // ﾐ｡ﾑびﾐｾﾐｺﾐｰ ﾐｲﾑ錦威ｵ
    uint16_t *row_plus = &image[imgdata.sizes.raw_width * 3 * (y + scale)];  // ﾐ｡ﾑびﾐｾﾐｺﾐｰ ﾐｽﾐｸﾐｶﾐｵ
...
void LibRaw::x3f_dpq_interpolate_af_sd(int xstart, int ystart, int xend, int yend, int xstep, int ystep, int scale)
{
  ...
  {
    uint16_t *row0 = &image[imgdata.sizes.raw_width * 3 * y];                // ﾐ斷ｰﾑ威ｰ ﾑ・びﾐｾﾐｺﾐｰ
    uint16_t *row1 = &image[imgdata.sizes.raw_width * 3 * (y + 1)];          // ﾐ｡ﾐｻﾐｵﾐｴﾑτ紗禍ｰﾑ・ﾑ・びﾐｾﾐｺﾐｰ
    uint16_t *row_minus = &image[imgdata.sizes.raw_width * 3 * (y - scale)]; // ﾐ｡ﾑびﾐｾﾐｺﾐｰ ﾐｲﾑ錦威ｵ
    uint16_t *row_plus =
        &image[imgdata.sizes.raw_width * 3 * (y + scale)]; // ﾐ｡ﾑびﾐｾﾐｺﾐｰ ﾐｽﾐｸﾐｶﾐｵ AF-point (scale=2 -> ﾐｽﾐｸﾐｶﾐｵ row1
